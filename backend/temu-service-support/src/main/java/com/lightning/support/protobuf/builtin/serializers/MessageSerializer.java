...
public class MessageSerializer extends ProtobufSerializer<MessageOrBuilder> {
  ...
  public void serialize(
    ...
    Include include = serializerProvider.getConfig().getDefaultPropertyInclusion()
        .getValueInclusion();
    ...
    PropertyNamingStrategyBase namingStrategy =
        new PropertyNamingStrategyWrapper(
    ...
    for (FieldDescriptor field : fields) {
      if (field.isRepeated()) {
        ...
        if (!valueList.isEmpty() || writeEmptyCollections) {
          ...
          } else if (valueList.size() == 1 && writeSingleElementArraysUnwrapped(
              serializerProvider)) {
            generator.writeFieldName(namingStrategy.translate(field.getName()));
      ...
      } else if (message.hasField(field) || (writeDefaultValues && !supportsFieldPresence(field)
